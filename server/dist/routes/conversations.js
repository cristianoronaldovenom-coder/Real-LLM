...
import { getTogetherClient, isTogetherModel, stripTogetherPrefix, MissingTogetherKeyError, } from "../lib/together.js";
import { getMistralClient, isMistralModel, stripMistralPrefix, MissingMistralKeyError, } from "../lib/mistral.js";
...
router.patch("/conversations/:id", async (req, res) => {
    ...
    if (body.model !== undefined)
        updates.model = normalizeModel(body.model);
    if (body.systemPrompt !== undefined)
        updates.systemPrompt = body.systemPrompt;
    if (body.useKnowledgeBase !== undefined)
        updates.useKnowledgeBase = body.useKnowledgeBase;
    if (body.useWebSearch !== undefined)
        updates.useWebSearch = body.useWebSearch;
...
router.post("/conversations/:id/messages", async (req, res) => {
    ...
    try {
        ...
        const stream = await client.chat.completions.create({ model: requestModel, messages: chatMessages, stream: true, max_tokens: maxTokens }, { signal: controller.signal });
        let full = "";
        ...
        res.end();
        if (full.trim())
            void learnFromExchange(body.content, full);
    }
    catch (err) {
        if (clientGone)
            return;
