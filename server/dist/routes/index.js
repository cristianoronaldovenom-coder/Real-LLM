...
router.get("/stats", async (_req, res) => {
    const [convCount] = await db.select({ count: sql `count(*)::int` }).from(conversations);
    const [msgCount] = await db.select({ count: sql `count(*)::int` }).from(messages);
    const [docCount] = await db.select({ count: sql `count(*)::int` }).from(documents);
    const [memCount] = await db.select({ count: sql `count(*)::int` }).from(memories);
    res.json({
