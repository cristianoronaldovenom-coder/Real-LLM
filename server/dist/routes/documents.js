...
router.get("/documents", async (_req, res) => {
    const rows = await db
        ...
        chunkCount: sql `(select count(*)::int from document_chunks where document_id = ${documents.id})`,
    })
        .from(documents)
...
router.get("/documents/:id", async (req, res) => {
    ...
    const [{ count }] = await db.select({ count: sql `count(*)::int` }).from(documentChunks).where(eq(documentChunks.documentId, doc.id));
