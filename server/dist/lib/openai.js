import OpenAI from "openai";
import { db, settings, SETTINGS_ROW_ID } from "../db.js";
import { eq } from "drizzle-orm";
const CACHE_TTL_MS = 30_000;
let cache = null;
async function readUserKey() {
    const [row] = await db
        .select({ key: settings.openaiApiKey })
        .from(settings)
        .where(eq(settings.id, SETTINGS_ROW_ID))
        .limit(1);
    const key = row?.key?.trim();
    return key ? key : null;
}
export function invalidateOpenAIKeyCache() {
    cache = null;
}
async function getUserKey() {
    const now = Date.now();
    if (cache && now - cache.fetchedAt < CACHE_TTL_MS)
        return cache.key;
    const key = await readUserKey();
    cache = { key, fetchedAt: now };
    return key;
}
export async function resolveOpenAIKey() {
    const userKey = await getUserKey();
    if (userKey)
        return { key: userKey, source: "user" };
    const envKey = process.env.OPENAI_API_KEY?.trim() || null;
    if (envKey)
        return { key: envKey, source: "env" };
    return { key: null, source: "none" };
}
export class MissingOpenAIKeyError extends Error {
    constructor() {
        super("No OpenAI API key is configured. Add your key on the Training or Settings page, or set the OPENAI_API_KEY env variable.");
        this.name = "MissingOpenAIKeyError";
    }
}
export async function getOpenAIClient() {
    const { key } = await resolveOpenAIKey();
    if (!key)
        throw new MissingOpenAIKeyError();
    return new OpenAI({ apiKey: key });
}
/**
 * OpenAI-hosted models (base + fine-tuned) are routed directly to OpenAI.
 * OpenRouter models always contain a "/" (e.g. "meta-llama/llama-3.3-70b-instruct").
 */
export function isOpenAIModel(model) {
    return (model.startsWith("ft:") ||
        model.startsWith("gpt-") ||
        /^o[1-9]/.test(model));
}
