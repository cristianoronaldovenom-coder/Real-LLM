...
function isOpenSource(id) {
    if (id.startsWith("google/"))
        return id.includes("gemma");
...
function cleanDescription(desc, name) {
    if (!desc?.trim())
        return `${name} — an open-source model available through OpenRouter.`;
...
async function fetchCatalog() {
    ...
    for (const m of json.data) {
        if (!m.id || curatedIds.has(m.id))
            continue;
        if (m.id.endsWith(":free"))
            continue;
        if (!isOpenSource(m.id))
            continue;
        if (!m.context_length)
            continue;
