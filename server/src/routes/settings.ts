...
function maskOpenRouter(key: string): string {
  return `sk-or-…${key.slice(-4)}`;
}

function maskOpenAI(key: string): string {
  return `sk-…${key.slice(-4)}`;
}

function maskTavily(key: string): string {
  return `tvly-…${key.slice(-4)}`;
}

function maskGeneric(key: string): string {
  return `…${key.slice(-4)}`;
}
...
async function setOpenRouterKey(key: string | null) {
  await db
    .insert(settings)
...
async function setOpenAIKey(key: string | null) {
  await db
    .insert(settings)
...
async function setTavilyKey(key: string | null) {
  await db
    .insert(settings)
...
async function setTogetherKey(key: string | null) {
  await db
    .insert(settings)
...
async function setMistralKey(key: string | null) {
  await db
    .insert(settings)
...
router.delete("/settings", async (req, res) => {
  ...
  if (provider === "openai") await setOpenAIKey(null);
  else if (provider === "tavily") await setTavilyKey(null);
  else if (provider === "together") await setTogetherKey(null);
  else if (provider === "mistral") await setMistralKey(null);
  else await setOpenRouterKey(null);
  res.json(await buildStatus());
});
