...
async function readUserKey(): Promise<string | null> {
  const [row] = await db
    .select({ key: settings.openaiApiKey })
    .from(settings)
...
export async function resolveOpenAIKey(): Promise<{ key: string | null; source: KeySource }> {
...
export class MissingOpenAIKeyError extends Error {
  constructor() {
    super(
      "No OpenAI API key is configured. Add your key on the Training or Settings page, or set the OPENAI_API_KEY env variable."
    );
...
/**
 * OpenAI-hosted models (base + fine-tuned) are routed directly to OpenAI.
