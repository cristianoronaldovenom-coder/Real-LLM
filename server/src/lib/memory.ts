...
export function buildMemoryBlock(items: string[]): string {
  if (items.length === 0) return "";
  const selected: string[] = [];
  let used = 0;
  for (const item of items) {
    const line = `- ${item}`;
    if (used + line.length > MEMORY_PROMPT_CHAR_BUDGET && selected.length > 0) break;
    selected.push(line);
    used += line.length + 1;
  }
  return (
    "Here is what you already know about the user from previous conversations. " +
...
export async function learnFromExchange(userText: string, assistantText: string): Promise<void> {
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You maintain a long-term memory of durable facts about a user. " +
