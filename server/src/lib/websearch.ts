...
export async function resolveTavilyKey(): Promise<{ key: string | null; source: KeySource }> {
 ...
 */
export async function performWebSearch(query: string, maxResults = 5): Promise<WebSearchResponse | null> {
...
export function buildWebSearchBlock(search: WebSearchResponse): string {
  const parts: string[] = [];
  if (search.answer) parts.push(`Quick answer: ${search.answer}`);
