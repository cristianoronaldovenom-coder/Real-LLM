...
export function invalidateTogetherKeyCache(): void {
  cache = null;
}
...
export async function resolveTogetherKey(): Promise<{ key: string | null; source: KeySource }> {
...
export class MissingTogetherKeyError extends Error {
  constructor() {
    super(
      "No Together AI API key is configured. Add your key on the Training or Settings page, or set the TOGETHER_API_KEY env variable."
    );
...
export function isTogetherModel(model: string): boolean {
  return model.startsWith(TOGETHER_PREFIX);
}

export function stripTogetherPrefix(model: string): string {
...
export async function getTogetherClient(): Promise<OpenAI> {
...
function mapStatus(raw: unknown): string {
  ...
  if (["error", "failed", "user_error"].includes(v)) return "failed";
  if (["cancelled", "canceled", "cancel_requested"].includes(v)) return "cancelled";
  if (["pending", "queued"].includes(v)) return "queued";
...
function normalizeJob(json: Record<string, unknown>): TogetherJob {
  const outputName =
    (json.output_name as string | undefined) ??
    (json.model_output_name as string | undefined) ??
    null;
  return {
    id: String(json.id),
...
export async function uploadTrainingFile(jsonl: string): Promise<string> {
  ...
  if (!json.id) throw new Error("Together AI did not return a file id for the upload.");
  return json.id;
}
...
export async function createFineTune(params: {
  ...
  if (hp?.nEpochs != null) body.n_epochs = hp.nEpochs;
  if (hp?.batchSize != null) body.batch_size = hp.batchSize;
  if (hp?.learningRate != null) body.learning_rate = hp.learningRate;
  if (hp?.loraR != null) body.lora_r = hp.loraR;
  if (hp?.loraAlpha != null) body.lora_alpha = hp.loraAlpha;
...
export async function retrieveFineTune(id: string): Promise<TogetherJob> {
  const key = await getKeyOrThrow();
