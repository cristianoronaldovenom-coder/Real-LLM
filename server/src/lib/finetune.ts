...
import { db, conversations, messages, trainingJobs, type TrainingJob } from "../db.js";
import { getOpenAIClient, resolveOpenAIKey, MissingOpenAIKeyError } from "./openai.js";
import {
  resolveTogetherKey,
  MissingTogetherKeyError,
  uploadTrainingFile as togetherUpload,
  createFineTune as togetherCreateFineTune,
  retrieveFineTune as togetherRetrieve,
  cancelFineTune as togetherCancel,
  TOGETHER_PREFIX,
} from "./together.js";
import {
  resolveMistralKey,
  MissingMistralKeyError,
  uploadTrainingFile as mistralUpload,
  createFineTune as mistralCreateFineTune,
  retrieveFineTune as mistralRetrieve,
  cancelFineTune as mistralCancel,
  MISTRAL_PREFIX,
} from "./mistral.js";
import type { AiModel } from "./models.js";

export type Provider = "openai" | "together" | "mistral";
...
export function getBaseModels(): BaseModelOption[] {
  return [
    ...FINE_TUNE_BASE_MODELS.map((m) => ({ ...m, provider: "openai" as const })),
    ...TOGETHER_BASE_MODELS.map((m) => ({ ...m, provider: "together" as const })),
    ...MISTRAL_BASE_MODELS.map((m) => ({ ...m, provider: "mistral" as const })),
  ];
}

function providerForBaseModel(id: string): Provider | null {
  if (FINE_TUNE_BASE_MODELS.some((m) => m.id === id)) return "openai";
  if (TOGETHER_BASE_MODELS.some((m) => m.id === id)) return "together";
  if (MISTRAL_BASE_MODELS.some((m) => m.id === id)) return "mistral";
  return null;
}

/** Together suffixes must be short and alphanumeric-ish; derive one from the name. */
function suffixFromName(name: string): string | undefined {
  const slug = name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
...
export interface Hyperparameters {
  nEpochs?: number;
  batchSize?: number;
  learningRate?: number; // Together AI / Mistral: absolute learning rate
  learningRateMultiplier?: number; // OpenAI: multiplier on its default
...
function exampleToMessages(ex: ExampleInput): ChatMsg[] {
  const msgs: ChatMsg[] = [];
  if (ex.system?.trim()) msgs.push({ role: "system", content: ex.system.trim() });
  msgs.push({ role: "user", content: ex.prompt.trim() });
  msgs.push({ role: "assistant", content: ex.completion.trim() });
  return msgs;
}

export function examplesToJsonl(examples: ChatMsg[][]): string {
 ...
 */
function normalizeAlternating(msgs: ChatMsg[]): ChatMsg[] {
  let system: ChatMsg | undefined;
  const merged: ChatMsg[] = [];
  for (const m of msgs) {
    if (m.role === "system") {
      if (!system) system = { role: "system", content: m.content };
      else system.content += "\n\n" + m.content;
      continue;
    }
...
export function buildFromExamples(examples: ExampleInput[]): ChatMsg[][] {
  return examples
    .filter((e) => e.prompt?.trim() && e.completion?.trim())
    .map(exampleToMessages);
}

export function buildFromJsonl(jsonl: string): ChatMsg[][] {
  const out: ChatMsg[][] = [];
  ...
  for (const line of lines) {
    ...
    const record = obj as { messages?: unknown };
    ...
    for (const raw of record.messages) {
      ...
      if (
        ...
        throw new TrainingValidationError(
          "Each message needs a role of system/user/assistant and string content."
        );
      }
...
export async function buildFromConversations(ids?: number[]): Promise<ChatMsg[][]> {
  const convos =
    ids && ids.length > 0
  ...
  for (const c of convos) {
    ...
    const chat: ChatMsg[] = [];
    if (c.systemPrompt?.trim()) chat.push({ role: "system", content: c.systemPrompt.trim() });
    for (const m of msgs) {
...
export async function buildExamples(params: CreateJobParams): Promise<ChatMsg[][]> {
  if (params.source === "conversations") return buildFromConversations(params.conversationIds);
  if (params.source === "jsonl") return buildFromJsonl(params.jsonl ?? "");
  return buildFromExamples(params.examples ?? []);
}

export async function createTrainingJob(params: CreateJobParams): Promise<TrainingJob> {
  const provider = providerForBaseModel(params.baseModel);
  if (!provider) {
    throw new TrainingValidationError(
      "Unsupported base model. Choose one of the listed fine-tunable models."
    );
  }
  ...
  if (examples.length < MIN_EXAMPLES) {
    throw new TrainingValidationError(
      `Fine-tuning needs at least ${MIN_EXAMPLES} examples. You provided ${examples.length}. ` +
  ...
  if (provider === "together") {
    ...
    if (!key) throw new MissingTogetherKeyError();
  ...
  if (provider === "mistral") {
    ...
    const mistralExamples = examples
      ...
      .filter(
        (m) => m.some((x) => x.role === "user") && m.some((x) => x.role === "assistant")
      );
    if (mistralExamples.length < MIN_EXAMPLES) {
      throw new TrainingValidationError(
        `After formatting for Mistral, only ${mistralExamples.length} usable example(s) remained ` +
  ...
  if (hp?.nEpochs != null) openaiHyper.n_epochs = hp.nEpochs;
  if (hp?.batchSize != null) openaiHyper.batch_size = hp.batchSize;
...
async function fetchProviderStatus(row: TrainingJob): Promise<StatusPatch> {
...
export async function syncTrainingJob(id: number): Promise<TrainingJob | null> {
  const [row] = await db.select().from(trainingJobs).where(eq(trainingJobs.id, id));
  if (!row) return null;
  if (!row.providerJobId || TERMINAL_STATUSES.has(row.status)) return row;
...
let activeSyncInFlight: Promise<void> | null = null;

export async function syncActiveTrainingJobs(force = false): Promise<void> {
  if (activeSyncInFlight) return activeSyncInFlight;
  const now = Date.now();
  if (!force && now - lastActiveSync < ACTIVE_SYNC_INTERVAL_MS) return;
...
export async function deleteTrainingJob(id: number): Promise<boolean> {
  const [row] = await db.select().from(trainingJobs).where(eq(trainingJobs.id, id));
  if (!row) return false;
...
export async function getFineTunedModels(): Promise<AiModel[]> {
  const rows = await db
    .select()
    .from(trainingJobs)
